...
class Test(object):
    ...
    def test(self):
        # we are loading all testing into the memory
        batch = Batch()
        test_batch_data, test_batch_labels = batch.minibatch(False)
        print ('Data Loaded!')

        # Build the graph for validation
        self.build_train_validation_graph()

        # Initialize a saver to restore checkpoint. Initialize a new session
        ...
        _, validation_error_value, validation_loss_value, validation_confusion_matrix =\
        sess.run([self.val_op, self.vali_top1_error, self.vali_loss, self.vali_confusion_matrix],\
     	{self.vali_image_placeholder: test_batch_data, self.vali_label_placeholder: test_batch_labels})

        print('        Testing Model    ')
        print('Testing top1 accuracy = %.4f' % (1 - validation_error_value))
        print('Testing loss = ', validation_loss_value)
        print('Testing confusion matrix(|Actual , --Predicted) = ')
